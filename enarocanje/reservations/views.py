...
def reservation(request, id):
    ...
    #service_provider_employee = get_object_or_404(ServiceProviderEmployee, service_id=id) #ServiceProviderEmployee.objects.filter(service_id=id)
    ...
    if request.method != 'POST':
        ...
        data = {'service_provider_id': service.service_provider_id, 'service_id': service.id} #, 'service_provider_employee_id': service_provider_employee.id
    ...
    if step == '3':
        ...
        form = ReservationForm(request, {'date': data.get('date'), 'time': data.get('time')}, workingHours=workingHours, service=service)
        ...
        if form.is_valid():
            ...
            reserve = Reservation(user=ruser, service=service, date=data['date'], time=data['time'], customer=customer)
            # Add backup fields
