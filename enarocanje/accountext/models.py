...
class ServiceProvider(models.Model):
	...
	timezone = models.CharField(_('timezone'), choices=((x, x,) for x in pytz.common_timezones), default='UTC', max_length=30)
	...
	web = models.CharField(_('web'), max_length=100)
	...
	def distance(self, lat, lng):
		return acos(sin(radians(lat)) * sin(radians(self.lat)) + cos(radians(lat)) * cos(radians(self.lat)) * cos(radians(lng) - radians(self.lng))) * 6378
	...
	def save(self, *args, **kwargs):
		...
		if address:
			r = requests.get('http://maps.googleapis.com/maps/api/geocode/json', params={'address': address, 'sensor': 'false'})
