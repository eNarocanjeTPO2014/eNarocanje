...
def mynotifications(request):
    if request.method == 'POST':
        ...
        emailist = []
        #posiljamo sporocilo vsem uporabnikom aplikacije, ki hocejo sprejemati obvestila o ponudbi
        #recipients = User.objects.filter(service_notifications=True)

        recipients = []
        serviceProviderID = request.user.service_provider_id
        reservations = Reservation.objects.filter(service_provider_id=serviceProviderID)
        for reservation in reservations:
            if reservation.user is None:
                recipient = User(email=reservation.user_email)
                recipients.append(recipient)
            else:
                if reservation.user.service_notifications:
        ...
        mail.send_many(emailist)
        emailForm = EmailForm()
        messages.success(request, _('Your messages was sent successfully.'))
        return render_to_response('notifications/mynotifications.html', locals(), context_instance=RequestContext(request))
...
@for_service_providers
def sendservicenotification(request):
    return render_to_response('notifications/sendservicenotification.html', locals(),
